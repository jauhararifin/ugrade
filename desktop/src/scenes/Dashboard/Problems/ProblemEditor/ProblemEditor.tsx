...
import { Problem, ProblemType } from 'ugrade/contest/store'
...
export const ProblemEditor: FunctionComponent<ProblemEditorProps> = ({
  problem,
  onSubmit,
}) => {
  const initialValue: ProblemFormValue = {
    ...
    type: problem ? problem.type : ProblemType.PROBLEM_TYPE_BATCH,
  ...
  const validationSchema = yup.object().shape({
    ...
    type: yup
      .number()
      .oneOf([
        ProblemType.PROBLEM_TYPE_BATCH,
        ProblemType.PROBLEM_TYPE_INTERACTIVE,
      ])
      .required(),
  ...
  const handleSubmit = async (
    values: ProblemFormValue,
    { setSubmitting }: FormikActions<ProblemFormValue>
  ) => {
    try {
      if (onSubmit) {
