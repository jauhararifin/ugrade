...
export interface AuthService {
  ...
  resetPassword(contestId: string, email: string, oneTimeCode: string, password: string): Promise<User>
  addUser(token: string, email: string, permissions: Permission[]): Promise<User>
  addContest(email: string, contestId: string): Promise<User>
  setMyPassword(token: string, oldPassword: string, password: string): Promise<User>
  setMyName(token: string, name: string): Promise<User>
  setPermissions(token: string, userId: string, permissions: Permission[]): Promise<User>
