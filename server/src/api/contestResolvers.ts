...
export type ContestByShortIdResolver = AppFieldResolver<any, { shortId: string }, Promise<Contest>>

export type ContestByUserResolver = AppFieldResolver<User, any, Promise<Contest>>
...
export type ContestByAnnouncementResolver = AppFieldResolver<Announcement, any, Promise<Contest>>
...
export const createContestResolvers = (contestService: ContestService): ContestResolvers => ({
  Query: {
    contestById: (_source, { id }) => wrap(contestService.getContestById(id)),
    contestByShortId: (_source, { shortId }) => wrap(contestService.getContestByShortId(shortId)),
  },
  Mutation: {
    createContest: (
      _source,
      { email, shortId, name, shortDescription, description, startTime, finishTime, permittedLanguageIds }
    ) =>
      wrap(
    ...
    setMyContest: (
      _source,
      { name, shortDescription, description, startTime, freezed, finishTime, permittedLanguageIds },
      { authToken }
    ) =>
      wrap(
