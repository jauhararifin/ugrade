...
import { TitleAction, titleReducer, TitleState } from 'ugrade/common/title/store'
import { ContestAction, contestReducer, ContestState } from 'ugrade/contest/store'
import { ServerStatusAction, serverStatusReducer, ServerStatusState } from 'ugrade/server/store'
...
import { SettingAction, settingReducer, SettingState } from 'ugrade/settings/store'
import { UserProfileAction, userProfileReducer, UserProfileState } from 'ugrade/userprofile/store'
...
export const createStore = (history: History, thunkExtraArguments: ThunkExtraArguments) => {
  const store = createReduxStore<AppState, AppAction, {}, {}>(
    ...
    compose(
      applyMiddleware(routerMiddleware(history), thunk.withExtraArgument(thunkExtraArguments), logger),
