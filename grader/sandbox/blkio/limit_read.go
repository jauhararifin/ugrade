...
// LimitRead make limiter for file reading in bytes per second. `file` should contain absolute path to host filesystem.
func (limiter *Limiter) LimitRead(file string, bps uint64) error {
	...
	// write blkio read throttle limit to `blkio.throttle.read_bps_device` inside cgroup
	cgroupBlkioPath := path.Join(cgroupPath, "blkio", limiter.Name)
	logrus.WithField("path", cgroupBlkioPath).WithField("bps", bps).Debug("write blkio read throttle")
	if err := ioutil.WriteFile(
		path.Join(cgroupBlkioPath, "blkio.throttle.read_bps_device"),
